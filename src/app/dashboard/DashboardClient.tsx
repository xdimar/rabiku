...
export default function DashboardClient({
  ...
  const [selectedPresetId, setSelectedPresetId] = useState<string>("minimalist");
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col font-sans text-stone-900">
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 bg-stone-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-3xl w-full p-6 sm:p-7 shadow-2xl border border-stone-200 animate-in fade-in zoom-in-95 duration-150 max-h-[92vh] flex flex-col">
            ...
            <form onSubmit={handleCreate} className="space-y-5 overflow-y-auto pr-1">
              ...
              {/* Sub-selector: Template Gallery Grid (jika mode preset) */}
              {creationMode === "preset" && (
                <div className="bg-stone-50/90 rounded-2xl p-4 border border-stone-200/80 space-y-3 animate-in fade-in duration-150">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                      <label className="block text-xs font-bold text-stone-900 uppercase tracking-wider">
                        Koleksi Desain Template Siap Pakai
                      </label>
                      <p className="text-[11px] text-stone-500">
                        Pilih gaya visual yang paling sesuai dengan impian pernikahan Anda
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-1 overflow-x-auto pb-1 sm:pb-0">
                      {[
                        ...
                        { id: "cultural", label: "Adat / Budaya" },
                        { id: "religious", label: "Islami" },
                      ].map((cat) => (
                        <button
                          key={cat.id}
                          type="button"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-72 overflow-y-auto pr-1">
                    {WEDDING_TEMPLATES.filter((tpl) =>
                      templateCategory === "all" ? true : tpl.category === templateCategory
                    ).map((tpl) => {
                      ...
                      return (
                        <div
                          ...
                          className={`group relative rounded-xl border-2 overflow-hidden transition-all cursor-pointer bg-white flex flex-col ${
                            isSelected
                              ? "border-stone-900 ring-2 ring-stone-900/20 shadow-md scale-[1.01]"
                        ...
                        >
                          ...
                          <div className="relative h-24 w-full overflow-hidden bg-stone-100">
                            ...
                            <span
                              className="absolute top-2 right-2 w-3.5 h-3.5 rounded-full border border-white shadow-xs"
                              style={{ backgroundColor: tpl.accentColor }}
                              title="Warna Aksen"
                            />
                            {isSelected && (
                              <div className="absolute top-2 left-2 px-1.5 py-0.5 rounded-md bg-stone-900 text-white text-[10px] font-bold flex items-center gap-1 shadow-sm">
