...
export default function WhatsAppSimulator() {
  ...
  return (
    <section
      ...
      className="py-24 px-6 bg-stone-50 border-b border-stone-200/70 relative overflow-hidden"
    >
      {/* Decorative ambient glow harmonized with Rabiku gold & stone palette */}
      <div className="absolute top-1/3 left-10 w-96 h-96 bg-amber-100/50 rounded-full blur-3xl -z-10 pointer-events-none" />
      <div className="absolute bottom-10 right-10 w-96 h-96 bg-stone-200/50 rounded-full blur-3xl -z-10 pointer-events-none" />

      <div className="max-w-6xl mx-auto">
        <ScrollFadeIn direction="up" className="text-center max-w-2xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-50/90 border border-amber-200/90 text-amber-900 mb-3 shadow-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Left: Interactive Controls Card */}
          <ScrollFadeIn
            direction="right"
            delay={0.1}
            className="lg:col-span-5 bg-white p-6 sm:p-8 rounded-3xl border border-stone-200/80 shadow-md shadow-stone-900/5 space-y-6"
          >
            <div>
            ...
            <div>
              ...
              <input
                ...
                maxLength={35}
                className="w-full px-4 py-2.5 text-sm bg-stone-50/80 border border-stone-200/90 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400 focus:bg-white text-stone-900 transition-all font-medium placeholder:text-stone-400 shadow-inner"
              />
              <div className="flex gap-1.5 mt-2.5 flex-wrap">
                {["Bpk. Hendra & Partner", "Sahabat Karib - Dian", "Keluarga Bpk. Rahmat"].map(
                  (preset, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => setGuestName(preset)}
                      className="text-[10px] px-2.5 py-1 rounded-lg bg-stone-100/80 hover:bg-stone-200/80 text-stone-600 font-medium border border-stone-200/60 transition-colors"
                    >
                      + {preset}
                    </button>
                  )
            ...
            <div className="pt-4 border-t border-stone-100 flex flex-col sm:flex-row gap-3">
              <button
                type="button"
                onClick={handleCopy}
                className="flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-stone-900 text-white text-xs font-semibold hover:bg-stone-800 transition-all shadow-sm active:scale-[0.98]"
              >
                {copied ? (
                  <>
              ...
              <Link
                ...
                target="_blank"
                className="flex items-center justify-center gap-1.5 py-3 px-4 rounded-xl bg-stone-100/90 border border-stone-200 text-stone-800 text-xs font-semibold hover:bg-stone-200/80 transition-colors shadow-2xs"
              >
                ...
                <div className="flex-1 p-3.5 overflow-y-auto space-y-3 relative">
                  ...
                  <div className="flex justify-end">
                    <div className="max-w-[92%] rounded-2xl rounded-tr-xs bg-[#d9fdd3] text-stone-900 p-2.5 shadow-sm border border-emerald-200/60 text-xs">
                      ...
                      >
                        <div className="h-28 bg-gradient-to-tr from-stone-950 via-stone-900 to-amber-900 p-3 flex flex-col justify-between text-white relative">
